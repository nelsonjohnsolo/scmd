...
`plugin/` holds a manifest and one command named `run`, exposed by Claude Code under the plugin namespace as `/scmd:run`. Its instruction is to run the scoped npm launcher in the background and report the URL. The plugin form is `npx -y --prefix="${CLAUDE_PLUGIN_ROOT}" --prefer-online @nelsonjohnsolo/scmd@latest`: the plugin root provides a trusted package-resolution context instead of the current project, while `@latest` plus `--prefer-online` selects the current published release. The repository root carries the marketplace manifest so `/plugin marketplace add <owner>/scmd` works. The plugin contains no product logic of its own; if the plugin format shifts, the tool is unaffected.

### D16. Visual system (approved 2026-09-19)
The page implements exactly this system; nothing else is introduced.

- **Palette (light):** paper `#FFFFFF`, ink `#121212`, muted `#6E6864`, soft surface `#F6F4F2`, red `#E5321E` (delete — the only loud destructive accent), green `#1F9D55` (keep — semantic, quiet), marker `#FFE84D` (headline highlight and the enabled final review CTA only). Dark theme swaps paper/ink (`#101010` / `#F4F2EF`) and lifts red/green (`#FF5A45` / `#3FCB7A`); marker-backed controls keep dark `#121212` text in both themes; every colour is a CSS token on `:root`, redefined under `prefers-color-scheme: dark` and `[data-theme="dark"]`.
...
- **Controls:** pill buttons (2px border, 999px radius) — primary is ink on paper, outline, red for delete, green for keep, ghost for undo. The enabled final review CTA is marker yellow with a dark ink border and text; its disabled state is neutral. Chips for filters (ink when on), mono uppercase badges for memory type, keycap glyphs for keyboard hints.
- **Deck layout, top to bottom:** top bar (wordmark, then a tightly grouped right-side Trash button with an inline icon and the final review CTA) → filters on the soft surface (projects, types, Unreviewed/Everything toggle, search) → a non-sticky progress row directly above the deck ("n of m", an ink-filled bar, and staged kept/deleted counts) → one centred card, max 560px → an action row with visible clearance below the decorative card edges (✗ Delete · ↑ Skip · ✓ Keep · Undo) → keyboard hints → rewrite box with a bordered "Edit by hand" button immediately below Rewrite on the right and the "uses your local Claude Code login" line → notices as a sticker toast beneath. The progress numerator counts staged keep/delete decisions within the current project/type/scope/search filters; the denominator is every memory in that filtered scope. Skip and edit do not advance it, undo reverses it, and filter changes recalculate it. Kept/deleted counts remain session-wide because the final review applies every staged decision. The normal "x memories in deck" status gives way to this row, while empty, error, and read-only instruction-hit messages remain visible.
- **Tone:** blunt and short; at most one joke per screen; no exclamation marks; controls say what happens (for example, "Review 17 decisions").
- **Motion:** Dragging tracks the pointer directly, tilts up to ±6°, and reveals the matching DELETE/KEEP stamp. A committed pointer drag continues from its release position and exits offstage in the same direction over 180 ms; a short or cancelled drag returns to rest over 180 ms. Delete/Keep buttons and the left/right keyboard shortcuts use the same directional choreography from the centred card, leaning briefly before completing the exit in at most 220 ms total, with the 150 ms stamp transition visible throughout. Skip lifts and fades upward over 180 ms with no decision stamp. Undo restores the card from the direction of its prior exit (left for delete, right for keep, above for skip) over 180 ms with no stale decision stamp; the next card may settle forward from the decorative deck edges over 120 ms. One motion coordinator owns pointer, button, and keyboard actions, locks decision inputs during exit/re-entry, commits the in-memory state only after an outgoing animation, and guards the animated card id against live rerenders. Under `prefers-reduced-motion`, every state change and focus update happens immediately with no transform, fade, stamp animation, or artificial wait.

### D17. Scoped npm identity, short executable and slash command
The public package is `@nelsonjohnsolo/scmd` because npm rejected the unscoped `scmd` name under its package-name similarity policy. The short direct no-install command remains `npx -y @nelsonjohnsolo/scmd`; the installed executable remains `scmd`, and the Claude Code command is `/scmd:run` because installed plugin commands are always namespaced. Both the plugin and the recommended terminal instructions use an isolated npm prefix plus `@latest` and `--prefer-online` so a package in the current project cannot shadow SCMD; the shorter direct form is documented only for trusted directories. Product copy leads with `/scmd:run` for the intended audience and presents terminal launch as the alternative.
